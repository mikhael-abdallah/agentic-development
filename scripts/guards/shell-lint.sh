...
# ShellCheck publishes no checksum file, so both hashes were recorded from
# the release asset. (Capitalised so this line is not read as a directive.) Pinning them still detects a re-uploaded asset and a
