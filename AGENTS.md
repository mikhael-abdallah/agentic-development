...
4. Auto-merge is on: a green PR merges itself; a red one is yours to fix. **After every push, run `scripts/watch-pr.sh <pr-number>` and act on its verdict** — it watches check *conclusions*, fails loudly listing the failed jobs, and only reports `MERGED` after confirming the merge. Never poll PR state (a failed check leaves the PR open forever, so that wait never ends) and never write ad-hoc watch loops.
